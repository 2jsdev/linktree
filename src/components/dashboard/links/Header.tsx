...
export default function Header() {
  ...
  const copyToClipboard = () => {
    if (data?.username) {
      const url = `https://wookiel.ink/${data.username}`;
      navigator.clipboard
        .writeText(url)
        .then(() => {
          setCopied(true);
      ...
      {data?.username ? (
        <>
          <div className="text-2xl font-bold text-center sm:text-left">
            <div>Your Wookielink is live:</div>
            <a
              href={`https://wookiel.ink/${data.username}`}
              target="_blank"
              rel="noopener noreferrer"
              className="text-primary hover:underline flex items-center justify-center sm:justify-start"
            >
              wookiel.ink/{data.username}
        ...
        <>
          <h1 className="text-2xl font-bold text-center sm:text-left">
            Your Wookielink is not live yet
          </h1>
          <Link href="/dashboard/profile">
            <Button variant="outline">Set up your Wookielink</Button>
          </Link>
        </>
      )}
    </header>
